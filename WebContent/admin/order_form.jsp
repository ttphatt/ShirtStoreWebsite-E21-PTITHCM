		...
		<div class="container mb-5 mt-5">
			...
			<form action="update_order" method="post">
					...
					<table border="1" cellpadding="10" style="text-align: center; width: 1200px" class="table table-striped table-bordered table-hover table-custom">
						...
						<c:forEach items="${order.orderDetails}" var="orderDetail" varStatus="status">
							<tr>
								...
								<td class="align-middle justify-content-center text-center">${orderDetail.size}</td>
								<td class="align-middle justify-content-center text-center">
				...
				<div align="center">
					...
					<%--		&nbsp;&nbsp;&nbsp;&nbsp;--%>
					<button type="button" class="btn custom-btn-return" onclick="javascript:window.location.href='list_order';">Back</button>
					&nbsp;&nbsp;&nbsp;&nbsp;
					<c:if test="${order.status == 'Processing'}">
						<a href="shipping_order?orderId=${order.orderId}" class="btn custom-btn-submit">Shipping</a>
					</c:if>
					<c:if test="${order.status == 'Shipping'}">
						<a href="delivered_order?orderId=${order.orderId}" class="btn custom-btn-cart">Delivered</a>
					</c:if>
				</div>
			</form>
		</div>
